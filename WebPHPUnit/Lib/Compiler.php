...
class Compiler {
    ...
    *  @return string
    */
    public static function compile($file, array $options) {
        //$stats = stat($file);
        $dir = dirname($file);
        ...
        $template = 'template_' . $location . '.html';
        ...
        if (
            ...
            || false === file_put_contents($template, $compiled)
        ) {
            return false;
        }
        ...
        foreach ( glob($pattern) as $old ) {
            if ( realpath($old) !== realpath($template) ) {
                unlink($old);
            }
        }
        return $template;
